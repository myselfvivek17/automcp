...
class CodeGeneratorAgent(BaseAgent):
    ...
    async def _generate_python_code(self, mcp_tools: List[Dict], auth_config: Dict, base_url: str) -> str:
        ...
        for tool in mcp_tools:
            ...
            for p in path_params:
                doc_lines.append(f'        {p}: Path parameter for {path}')
            for p in query_params:
                doc_lines.append(f'        {p}: Query parameter (optional)')
            if has_body:
                doc_lines.append('        body: Request body as a dictionary')
            doc_lines.append('    """')

            lines += ['@mcp.tool()']
            lines += [f'async def {fn}({sig}) -> str:']
            lines += doc_lines
            ...
            call = f'client.{method}(url, headers=headers, params=query or None'
            if has_body:
                call += ', json=body'
            call += ')'
            lines += [
    ...
    async def _generate_typescript_code(self, mcp_tools: List[Dict], auth_config: Dict, base_url: str) -> str:
        ...
        for tool in mcp_tools:
            ...
            # Build Zod inputSchema entries
            schema_entries = []
            handler_args = []
            for p in path_params:
                pname = p.get("name", "param")
                pdesc = p.get("description", f"Path parameter: {pname}")
                schema_entries.append(f'    {pname}: z.string().describe("{pdesc}"),')
                handler_args.append(pname)
            for p in query_params:
                pname = p.get("name", "param")
                pdesc = p.get("description", f"Query parameter: {pname}")
                schema_entries.append(f'    {pname}: z.string().optional().describe("{pdesc}"),')
                handler_args.append(pname)
            if has_body:
            ...
            # Build query string for query params
            ...
            if query_params:
                ...
                lines += [
                    ...
                    f'    const qparams: Record<string, string | undefined> = {{ {qparts} }};',
                    f'    for (const [k, v] of Object.entries(qparams)) if (v) query.set(k, v);',
                    f'    const fullUrl = query.toString() ? `${{url}}?${{query}}` : url;',
                ]
                req_url = 'fullUrl'
            else:
                req_url = 'url'
            ...
            lines += [
                ...
                f'    const data = await r.text();',
                f'    return {{ content: [{{ type: "text", text: data }}] }};',
                f'  }}',
                f');',
                '',
            ]
